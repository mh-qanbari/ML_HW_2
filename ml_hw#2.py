...
def main():
    ...
    with open(g_SMS_SPAM_FILE_ADDRESS, 'rb') as smsSpamFile:
        ...
        for row in spreader:
            ...
            text_words = [w.replace(' ', '') for w in text.split(' ') if len(w) > 1]
    ...
    print "Spliting test data from train data"
    ...
    while text_index < len(test_texts):
        indexes_count = len(words_index)
        # P(spam | feature)
        p_spam = 1
        # P(ham  | feature)
        p_ham  = 1
        for f_index in range(indexes_count):
            # f_max = words_count[f_index]
            # f_value = 1
            # while f_value <= f_max:
            p_h, p_s = probability(words_index[f_index], test_texts.count(words_index[f_index]),
                                   train_sms_texts, train_sms_types)
            p_ham *= p_h
            p_spam *= p_s
        ...
        if (p_ham / p_spam) >= 1:
            output_types.append(g_HAM_WORD + " (" + test_types[text_index] + ")")
        else:
            output_types.append(g_SPAM_WORD + " (" + test_types[text_index] + ")")
